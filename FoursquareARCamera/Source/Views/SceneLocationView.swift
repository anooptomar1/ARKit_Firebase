...
public class SceneLocationView: ARSCNView, ARSCNViewDelegate {
    ...
    func setupFocusSquare() {
        ...
        let serialScheduler = SerialDispatchQueueScheduler(internalSerialQueueName: "Test.ARKit.CoreLocation.serialSceneKitQueue")
        Observable.just(1, scheduler: serialScheduler)
            .subscribe {
                self.focusSquare?.isHidden = true
                self.focusSquare?.removeFromParentNode()
                self.focusSquare = FocusSquare()
                self.scene.rootNode.addChildNode(self.focusSquare!)
            }
...
//        if virtualObject != nil && self.isNode(virtualObject!, insideFrustumOf: self.pointOfView!) {
//            focusSquare?.hide()
//        } else {
            focusSquare?.unhide()
//        }
    ...
    func worldPositionFromScreenPosition(_ position: CGPoint,
        ...
        hitAPlane: Bool) {
            ...
            let highQualityfeatureHitTestResults =
                self.hitTestWithFeatures(position, coneOpeningAngleInDegrees: 18, minDistance: 0.2, maxDistance: 2.0)
            ...
            if (infinitePlane) || !highQualityFeatureHitTestResult {
                ...
                let pointOnInfinitePlane = self.hitTestWithInfiniteHorizontalPlane(position, pointOnPlane)
    ...
    func updatePosition(object: VirtualObject) {
        guard let currentPosition = currentScenePosition(),
        ...
        let locationTranslation = currentLocation.translation(toLocation: location)
